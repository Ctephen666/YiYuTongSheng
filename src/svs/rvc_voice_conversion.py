...
def apply_rvc_voice_conversion(config: dict, input_audio: str | Path | None = None) -> dict:
    ...
    report_path = _config_path(config, "rvc_report", "data/svs/rvc_voice_conversion_report.json")
    temp_output = output_audio.with_name(f"{output_audio.stem}_rvc_tmp{output_audio.suffix}")
    input_path = _resolve(config, input_audio) if input_audio else output_audio
    ...
    if status == "success":
        shutil.move(str(temp_output), str(output_audio))
        ensure_parent(svc_audio)
        shutil.copy2(output_audio, svc_audio)
    report = {
        "format": "rvc_voice_conversion_report.v1",
        "status": status,
        "enabled": True,
        "converted": converted,
